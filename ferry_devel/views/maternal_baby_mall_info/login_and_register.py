...
def register(request):
    return render_to_response("maternal_baby_mall_info/html/register.html",locals())
...
def register_handle(request):
    ...
    if(user_name == "" or user_pwd == ""):
        request_data['register_message'] = "用户名和密码都不能为空"
        return render(request, 'maternal_baby_mall_info/html/register.html', request_data)
    if(len(user_name) < 4 or len(user_pwd) < 6):
        request_data['register_message'] = "用户名或密码长度不符合要求"
        return render(request, 'maternal_baby_mall_info/html/register.html', request_data)
    ...
    try:
        ...
        if len(result_sql_1) == 1:
            ...
            result_data['register_message'] = "用户名已存在"
            return render(request,'maternal_baby_mall_info/html/register.html',result_data)
        else:
            # 执行insert into语句
            cursor.execute(sql)
            # 提交到数据库执行
...
def login_handle(request):
    ...
    try:
        ...
        result_data = {}
        if (user_name == "" or user_pwd == ""):
            result_data['login_message'] = "用户名和密码都不能为空"
            return render(request, 'maternal_baby_mall_info/html/login.html', result_data)
        if len(results) == 1:
            result_data["code"] = '登录成功'
            for row in results:
            ...
            return render(request, 'maternal_baby_mall_info/html/index.html', result_data)
        else:
            #return HttpResponse('用户名或密码不正确')
            result_data['login_message'] = "用户名或密码不正确"
            return render(request, 'maternal_baby_mall_info/html/login.html', result_data)
    except:
        # 如果发生错误则回滚
        traceback.print_exc()
